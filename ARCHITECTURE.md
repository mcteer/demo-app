...
> microservice. Build it exactly to the state described here — **tier-4 (Vault dynamic secrets)** —
> with credentials delivered by the Vault Secrets Operator.
...
variables (standing in for the VSO-managed Secret in local development). After
